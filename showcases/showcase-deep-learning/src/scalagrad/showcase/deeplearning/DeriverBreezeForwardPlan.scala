            ...
            xs => 
                ...
                DenseVector((
                    for (i <- 0 until xs.length) 
                        yield f(
                            (xs, oneHotDenseVector(i, xs.length))
                        ).dv
                    ).toArray
                )
        ...
        override def derive(f: fT): dfT =
            (v1, m, s, v2) => 
                ...
                def oneHotDenseMatrix(i: Int, mrows: Int, mcols: Int): DenseMatrix[Double] = 
                    ...
                    res
                val v1Dummy = (v1, DenseVector.zeros[Double](v1.length))
                val mDummy = (m, DenseMatrix.zeros[Double](m.rows, m.cols))
                val sDummy = DualNumber(s, 0.0) 
                val v2Dummy = (v2, DenseVector.zeros[Double](v2.length))

                val v1Res =
                    DenseVector((
                        for (i <- 0 until v1.length) 
                            yield f(
                                (v1, oneHotDenseVector(i, v1.length)),
                                mDummy,
                                sDummy,
                                v2Dummy
                            ).dv
                ...
                val mRes = 
                    DenseMatrix((
                        for (i <- 0 until m.rows * m.cols) 
                            yield f(
                                v1Dummy,
                                (m, oneHotDenseMatrix(i, m.rows, m.cols)),
                                sDummy,
                                v2Dummy
                            ).dv
                        ).toArray
                ...
                val sRes = f(v1Dummy, mDummy, DualNumber(s, 1.0), v2Dummy).dv
                    ...
                    DenseVector((
                        for (i <- 0 until v2.length) 
                            yield f(
                                ...
                                mDummy,
                                sDummy,
                                (v2, oneHotDenseVector(i, v2.length))
                            ).dv
                        ).toArray
                    )
