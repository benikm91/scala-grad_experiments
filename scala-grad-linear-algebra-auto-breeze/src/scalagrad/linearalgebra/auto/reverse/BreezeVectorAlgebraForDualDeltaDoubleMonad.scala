...
import scalagrad.linearalgebra.auto.reverse.dualMonad.DeltaState.deltaLet
...
object BreezeVectorAlgebraForDualDeltaDoubleMonad extends BreezeVectorAlgebraForDualDouble:
    ...
    override type Scalar = DualDeltaScalarMonad[Double]
    override type ColumnVector = DualDeltaColumnVectorMonad[Double]
    override type RowVector = DualDeltaRowVectorMonad[Double]
    override type Matrix = DualDeltaMatrixMonad[Double]

    override given cd: CreateDual[Double, ScalarD, Scalar] = DualDeltaScalarMonad.create

    def createColumnVector(value: DenseVector[Double], dual: ColumnVectorD): ColumnVector = DualDeltaColumnVectorMonad(value, dual)
 
    def createRowVector(value: Transpose[DenseVector[Double]], dual: RowVectorD): RowVector = DualDeltaRowVectorMonad(value, dual)

    def createMatrix(value: DenseMatrix[Double], dual: MatrixD): Matrix = DualDeltaMatrixMonad(value, dual)
    ...
    def timesSDS(s: Double, ds: ScalarD): ScalarD = 
        for {
            ds <- ds
            newId <- deltaLet(DeltaScalar.Scale(s, ds))
        } yield DeltaScalar.Val(newId)
    ...
    override def elementWiseOpsM(m: Matrix, f: Scalar => Scalar): Matrix =
        createMatrix(
            new DenseMatrix(m.rows, m.cols, m.v.map(x => f(DualDeltaScalarMonad(x, DeltaMonad.zeroS)).v).toArray),
            for {
                dm <- m.dv
                id <- deltaLet(DeltaMatrix.ElementWiseOpsMonad(m.v, dm, f))
    ...
    override def rowWiseOpsM(m: Matrix, op: RowVector => RowVector): Matrix =
        ...
        createMatrix(
            ...
            for {
                dm <- m.dv
                newId <- deltaLet(DeltaMatrix.RowWiseOpsMonad(m.v, dm, op))
            } yield DeltaMatrix.Val(newId)
        )
    ...
    override def elementWiseOpsCV(v: ColumnVector, f: Scalar => Scalar): ColumnVector = 
        createColumnVector(
            new DenseVector(v.v.map(x => f(DualDeltaScalarMonad(x, DeltaMonad.zeroS)).v).toArray),
            for {
                dv <- v.dv
                id <- deltaLet(DeltaColumnVector.ElementWiseOpsMonad(v.v, dv, f))
    ...
    def liftMatrix(m: DenseMatrix[Double]): Matrix = 
        DualDeltaMatrixMonad(m, DeltaMonad.zeroM)