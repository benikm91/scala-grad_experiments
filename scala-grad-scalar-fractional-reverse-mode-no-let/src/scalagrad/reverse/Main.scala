...
object Main {
    def main(args: Array[String]): Unit = {
        ...
        val df1 = ScalaGrad.derive(f1[DeriverFractionalReverse.DNum[Double]])
        println(df1(1.0))
        ...
        def f2[T](x1: T, x2: T)(using f: Fractional[T]): T = 
            ...
            x1 * x2
        import DeriverFractionalReverse.given
        val df2 = ScalaGrad.derive(f2[DeriverFractionalReverse.DNum[Double]])
        println(df2(1.0, 2.0))
        ...
        def f3[T](xs: Vector[T])(using f: Fractional[T]): T = 
            import f._
            xs.reduce(_ * _)
        val df3 = ScalaGrad.derive(f3[DeriverFractionalReverse.DNum[Double]])
